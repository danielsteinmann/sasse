...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        rechnungsbuero = Group(name=u'Rechnungsbüro')
        rechnungsbuero.save()
        for perm in [
                'add_mitglied',
                'change_mitglied',
            ...
            rechnungsbuero.permissions.add(p)
        rechnungsbuero.save()
        #
        a0 = User.objects.create_user('A0', 'thomasbuehlmann@pontonier.ch', 'tom-bunn')
        a0.first_name = 'Posten'
        a0.last_name = 'A0'
        a0.groups.add(rechnungsbuero)
        a0.save()
        #
        noten = User.objects.create_user('Noten', 'thomasbuehlmann@pontonier.ch', 'tom-bunn')
        noten.first_name = 'Eingabe'
        noten.last_name = 'Noten'
        noten.groups.add(rechnungsbuero)
        noten.save()
    
    def backwards(self, orm):
        User.objects.filter(username='Noten').delete()
        User.objects.filter(username='A0').delete()
        Group.objects.filter(name=u'Rechnungsbüro').delete()
