...
def load_einzelfahren(wettkampf, disziplin, posten, csvfile):
    ...
    for row in zeiten:
        ...
        try:
            ...
            failed[startnr] = "Startnummer nicht gefunden"
            continue
        ...
        if not form.is_valid():
            failed[startnr] = "Ungültiges Zeitformat: {0}".format(zeit_str)
            continue
        ...
        if created:
            ...
            failed[startnr] = "Bereits eine Zeit vorhanden: {0}".format(b)
    failed_sorted = sorted(list(failed.items()), key=operator.itemgetter(0))
...
def load_sektionsfahren(p1, p2, formset, csvfile):
    ...
    for form in formset.forms:
        ...
        for s in Schiffsektion.objects.filter(gruppe=row['gruppe_id']):
            for posten in (p1, p2):
                ...
                if zeit_str is None:
                    failed[startnr] = "Keine Zeit im File gefunden"
                    continue
                form = BewertungForm(data={'wert': zeit_str}, **kwargs)
                if not form.is_valid():
                    failed[startnr] = "Ungültiges Zeitformat: {0}".format(zeit_str)
    ...
    for startnr in unused:
        failed[startnr] = "Zeit keinem Schiff zugeteilt"
