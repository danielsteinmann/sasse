...
class DoppelstarterEinzelTest(TestCase):
    ...
    def testRegulaererDoppelstarter(self):
        ...
        self.assertEquals(1, len(ds[0]['doppel']))
        self.assertEquals(1, len(ds[0]['normal']))
        self.assertEquals(1, len(ds[1]['doppel']))
        self.assertEquals(1, len(ds[1]['normal']))
    ...
    def testSortiertNachDoppelstarternummer(self):
        ...
        ds = read_doppelstarter(self.wettkampf, self.einzelfahren)
        self.assertEquals(2, len(ds))
        self.assertEquals(151, ds[0]['doppel'][0]['startnummer'])
        self.assertEquals(153, ds[1]['doppel'][0]['startnummer'])
    ...
    def testNichtMarkierteDoppelstarter(self):
        ...
        ds = read_doppelstarter(self.wettkampf, self.einzelfahren)
        self.assertEquals(2, len(ds))
        self.assertEquals(0, len(ds[0]['doppel']))
        self.assertEquals(2, len(ds[0]['normal']))
        self.assertEquals(0, len(ds[1]['doppel']))
        self.assertEquals(2, len(ds[1]['normal']))
    ...
    def testMarkiertAberNichtDoppelstarter(self):
        ...
        self.assertEquals(2, len(ds))
        self.assertEquals(1, len(ds[0]['doppel']))
        self.assertEquals(0, len(ds[0]['normal']))
        self.assertEquals(1, len(ds[1]['doppel']))
        self.assertEquals(0, len(ds[1]['normal']))
