...
class MitgliedImportTest(TestCase):
    ...
    def testMitgliedGefunden(self):
        ...
        self.assertEquals(self.steinmann, mitglied)
    ...
    def testMitgliedGeaendert(self):
        ...
        mitglied = cru_mitglied("STEUERMANN", row)
        self.assertEquals(mitglied.id, self.steinmann.id)
        from_db = Mitglied.objects.get(pk=mitglied.id)
        self.assertEquals(anderer_name, from_db.name)
    ...
    def testMitgliedErzeugt(self):
        ...
        mitglied = cru_mitglied("STEUERMANN", row)
        from_db = Mitglied.objects.get(pk=mitglied.id)
        self.assertEquals("Muster", from_db.name)


class SchiffeinzelImportTest(TestCase):
    def setUp(self):
        cache.clear()
        self.kat_D = Kategorie.objects.get(name=u"D")
        self.bremgarten = Sektion.objects.get(name=u"Bremgarten")
        self.testcup = Wettkampf.objects.create(
                name=u"Fallbaumcup",
                zusatz=u"Bremgarten",
                von=datetime.date(2011, 5, 14))
        self.einzel_gross = self.testcup.disziplin_set.create(name=u"Einzelfahren-Gross")
        einzel_klein = self.testcup.disziplin_set.create(name=u"Einzelfahren-Klein")
        self.steinmann = Mitglied.objects.create(
                nummer="10043",
                name=u"Steinmann",
                vorname=u"Daniel",
    ...
    def testUnveraendert(self):
        ...
        self.assertEquals(stats["unchanged"], 1)
    ...
    def testGeandert(self):
        ...
        self.csvfile.seek(0)
        stats = load_startliste(self.testcup, self.csvfile)
        self.assertEquals(stats["update"], 1)
    ...
    def testEingefuegt(self):
        ...
        self.csvfile.seek(0)
        stats = load_startliste(self.testcup, self.csvfile)
        self.assertEquals(stats["insert"], 1)

    def testWettkampfExistiertNicht(self):
        self.row['WETTKAMPF_NAME'] = "BlaBla"
        self.csvwriter.writerow(self.row)
        self.csvfile.seek(0)
        with self.assertRaisesRegexp(StartlisteImportException, 'Eingelesene Startliste.*'):
    ...
    def testDisziplinExistiertNicht(self):
        ...
        self.csvwriter.writerow(self.row)
        self.csvfile.seek(0)
        with self.assertRaisesRegexp(StartlisteImportException, u'BlaBla: Wettkampf/Disziplin nicht'):
            load_startliste(self.testcup, self.csvfile)
    ...
    def testSektionExistiertNicht(self):
        ...
        self.csvwriter.writerow(self.row)
        self.csvfile.seek(0)
        with self.assertRaisesRegexp(StartlisteImportException, u'BlaBla: Keine solche Sektion'):
            load_startliste(self.testcup, self.csvfile)
    ...
    def testKategorieExistiertNicht(self):
        ...
        self.csvwriter.writerow(self.row)
        self.csvfile.seek(0)
        with self.assertRaisesRegexp(StartlisteImportException, 'Z: Keine solche Kategorie'):
            load_startliste(self.testcup, self.csvfile)

    def testFalscheKolonnen(self):
        csvfile = StringIO.StringIO(u"""\
