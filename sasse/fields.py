...
class PunkteField(DecimalField):
    ...
    def clean(self, value):
        ...
        if self.gueltige_werte and value not in self.gueltige_werte:
            msg = 'Nur folgende Zahlen sind erlaubt: %s' % self.gueltige_werte_str
...
class ZeitInSekundenField(Field):
    ...
    default_error_messages = {
        'invalid': ("Ungültige Eingabe. Bitte Zeit im Format"
                    " 'min:sek.hundertstel' eingeben."
                    " Beispiel: '2:35.76' für 2 Minuten, 35 Sekunden und"
                    " 76 Hunderstel. Der Wert '2.35.76' ist auch ok.")
    }
...
class MitgliedSearchField(ModelChoiceField):
    ...
    def clean(self, value):
        ...
        if q.count() == 0:
            text = "Mitglied '%s' nicht gefunden. Bitte 'Name Vorname' oder 'Mitgliedernummer' eingeben" % (value,)
            raise ValidationError(text)
        elif q.count() > 1:
            text = "'%s' ist nicht eindeutig, bitte auswählen" % (value,)
            self.widget = Select()
            self.queryset = q
            raise ValidationError(text)
        else:
    ...
    def value_for_form(self, mitglied):
        ...
        if mitglied:
            ...
            result = " ".join([mitglied.nummer, mitglied.name, mitglied.vorname])
...
class StartnummernSelectionField(RegexField):
    ...
    """
    help_text = "Beispiele: '1-6,9' oder '600-'"
    default_error_messages = {
        'invalid': "Bitte nur ganze Zahlen, Bindestrich oder Komma eingeben",
    }
    ...
    def clean(self, value):
        ...
        if value and value.strip():
            ...
            for c in commas:
                if c == '':
                    text = "Ein Komma ohne Zahl links und rechts ist nicht gültig."
                    raise ValidationError(text)
                dashes = c.split('-')
                if len(dashes) == 1:
                    queries.append(Q(startnummer=int(c)))
                elif len(dashes) > 2:
                    text = "'%s' enthält mehr als einen Gedankenstrich." % (c,)
                    raise ValidationError(text)
                else:
                    from_nr = dashes[0]
                    until_nr = dashes[1]
                    if from_nr == '' and until_nr == '':
                        text = "Ein Gedankenstrich ohne Zahl links oder rechts ist nicht gültig."
                        raise ValidationError(text)
                    elif until_nr == '':
            ...
            if not result:
                text = "Keine Schiffe mit dieser Eingabe von Startnummern gefunden."
                raise ValidationError(text)
