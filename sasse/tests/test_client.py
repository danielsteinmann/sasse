...
class WettkampfPageTest(TestCase):
    ...
    def test_list(self):
        ...
        self.assertEqual(len(response.context[-1]['wettkaempfe']), 3)

    def test_list_by_year(self):
        listURL = reverse(wettkaempfe_by_year, args=['2008'])
        response = self.client.get(listURL)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context[-1]['year'], '2008')
        self.assertEqual(len(response.context[-1]['wettkaempfe']), 2)
...
class PostenPageTest(TestCase):
    ...
    def test_list(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context[-1]['posten']), 1)
...
class EinzelfahrenStartlistePageTest(TestCase):
    ...
    def test_neuer_steuermann(self):
        ...
        response = self.client.post(addURL, {
            'startnummer': '2',
            'steuermann_neu': 'True',
            'steuermann-name': 'Muster',
            'steuermann-vorname': 'Felix',
            'steuermann-jahrgang': '',
    ...
    def test_neuer_vorderfahrer(self):
        ...
        response = self.client.post(addURL, {
            'startnummer': '2',
            'vorderfahrer_neu': 'True',
            'vorderfahrer-name': 'Muster',
            'vorderfahrer-vorname': 'Felix',
            'vorderfahrer-jahrgang': '',
...
class PostenblattPageTest(TestCase):
    ...
    def test_naechster_posten(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual('F', response.context['posten_next_name'])
...
class RichtzeitPageTest(TestCase):
    ...
    def test_bestzeiten(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Die besten Zeiten für Posten E-F:')
