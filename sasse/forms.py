...
class WettkampfForm(ModelForm):
    name = UnicodeSlugField(
            help_text="Beispiele: 'Fällbaumcup' oder 'Wallbach'")
    zusatz = CharField(
...
class DisziplinForm(ModelForm):
    INITIAL_NAME = 'automatisch-gefüllt'
    name = UnicodeSlugField(
        ...
        if schiffe is not None:
            ...
            if sektion is not None:
                ...
                if self.sektion_check and not schiffe:
                    text = "Keine Schiffe mit diesen Suchkritierien gefunden."
...
class SchiffeinzelListForm(Form):
    ...
    def _mitglied_neu_form(self, position, flag_field):
        if flag_field not in self.data:
    ...
    def clean(self):
        ...
        if steuermann == vorderfahrer:
            text = "Steuermann kann nicht gleichzeitig Vorderfahrer sein"
            raise ValidationError(text)
        # Sektion
        sektion = self.cleaned_data.get('sektion')
        if sektion is None:
        ...
        if hinten_ds and vorne_ds:
            text = "Ein Schiff darf nicht aus zwei Doppelstartern bestehen."
            raise ValidationError(text)
        # Instanz erzeugen
...
class BewertungBaseFormSet(BaseFormSet):
    ...
    def _construct_form(self, i, **kwargs):
        ...
        if self.initial and 'is_checksum' in self.initial[i]:
    ...
    def clean(self):
        ...
        if checksum_eingabe != checksum_calc:
            msg = "Die Zahl %s erwartet" % (checksum_calc,)
